...
var $author$project$Main$GotEntities = function (a) {
	return {$: 'GotEntities', a: a};
};
...
var $author$project$Api$Endpoint$url = function (paths) {
	return $author$project$Api$Endpoint$Endpoint(
		A2($elm$url$Url$Builder$absolute, paths, _List_Nil));
};
var $author$project$Api$Endpoint$entries = $author$project$Api$Endpoint$url(
	_List_fromArray(
		['entries']));
...
var $author$project$Api$genericExpectJson = F2(
	function (toMsg, decodeRes) {
		return A2(
			...
			function (response) {
				switch (response.$) {
					...
					case 'Timeout_':
						return $elm$core$Result$Err('Time out');
					case 'NetworkError_':
					...
					default:
						var metadata = response.a;
						var body = response.b;
						var _v1 = A2($elm$json$Json$Decode$decodeString, decodeRes, body);
						if (_v1.$ === 'Ok') {
							var value = _v1.a;
							...
							var err = _v1.a;
							return $elm$core$Result$Err(
								'Bad Body: ' + $elm$json$Json$Decode$errorToString(err));
						}
				}
			});
	});
...
var $author$project$Api$get = F3(
	function (url, toMsg, decoder) {
		return $elm$http$Http$request(
			{
				body: $elm$http$Http$emptyBody,
				expect: A2($author$project$Api$genericExpectJson, toMsg, decoder),
				headers: _List_Nil,
				method: 'GET',
				timeout: $elm$core$Maybe$Nothing,
				tracker: $elm$core$Maybe$Nothing,
				url: $author$project$Api$Endpoint$unwrap(url)
			});
	});
var $author$project$Api$entries = function (toMsg) {
	return A2($author$project$Api$get, $author$project$Api$Endpoint$entries, toMsg);
};
...
var $author$project$Main$entriesDecoder = function () {
	var versionDec = $elm$json$Json$Decode$list($elm$json$Json$Decode$string);
	var entryDec = A3(
		$elm$json$Json$Decode$map2,
		F2(
			function (n, vs) {
				return _Utils_Tuple2(n, vs);
			}),
		A2($elm$json$Json$Decode$field, 'name', $elm$json$Json$Decode$string),
		A2($elm$json$Json$Decode$field, 'versions', versionDec));
	var entriesDec = $elm$json$Json$Decode$list(entryDec);
	var dec = A2(
...
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{entries: $elm$core$Dict$empty, is_loading: false, n_ticks: 0, _package: $elm$core$Maybe$Nothing, version: $elm$core$Maybe$Nothing},
		A2($author$project$Api$entries, $author$project$Main$GotEntities, $author$project$Main$entriesDecoder));
};
...
var $author$project$Main$ReloadDone = function (a) {
	return {$: 'ReloadDone', a: a};
};
var $elm$core$Platform$Cmd$batch = _Platform_batch;
var $elm$core$Platform$Cmd$none = $elm$core$Platform$Cmd$batch(_List_Nil);
var $elm$json$Json$Encode$string = _Json_wrap;
var $author$project$Main$openDoc = _Platform_outgoingPort('openDoc', $elm$json$Json$Encode$string);
var $author$project$Api$Endpoint$reload = $author$project$Api$Endpoint$url(
	_List_fromArray(
		['reload']));
var $author$project$Api$reload = $author$project$Api$get($author$project$Api$Endpoint$reload);
var $author$project$Main$reloadResDecoder = $elm$json$Json$Decode$succeed(_Utils_Tuple0);
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'GotEntities':
				if (msg.a.$ === 'Ok') {
					var entities = msg.a.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{entries: entities, _package: $elm$core$Maybe$Nothing, version: $elm$core$Maybe$Nothing}),
			...
			case 'SelectPackage':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							_package: $elm$core$Maybe$Just(n)
						}),
					$elm$core$Platform$Cmd$none);
			case 'SelectVersion':
				var v = msg.a;
			...
			case 'Reload':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{is_loading: true, n_ticks: 1}),
					A2($author$project$Api$reload, $author$project$Main$ReloadDone, $author$project$Main$reloadResDecoder));
			case 'ReloadDone':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{is_loading: false, n_ticks: 0}),
					A2($author$project$Api$entries, $author$project$Main$GotEntities, $author$project$Main$entriesDecoder));
			default:
				return model.is_loading ? _Utils_Tuple2(
					_Utils_update(
						model,
						{n_ticks: model.n_ticks + 1}),
...
var $author$project$Main$viewHeader = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('d-flex flex-column flex-md-row align-items-center p-3 px-md-4 mb-5 bg-white border-bottom shadow-sm')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$text('DocServer')
					]))
			]));
};
var $author$project$Main$SelectPackage = function (a) {
...
var $author$project$Main$viewPackageNames = function (model) {
	var go = F2(
		function (key, acc) {
			return A2(
				$elm$core$List$cons,
						...
						[
							...
							_List_fromArray(
								[
									...
									$author$project$Main$SelectPackage(key))
								]))
						]),
					_List_fromArray(
						[
	...
	var eHtml = A3(
		...
		go,
		_List_Nil,
		$elm$core$Dict$keys(model.entries));
	return $rundis$elm_bootstrap$Bootstrap$ListGroup$ul(eHtml);
};
var $author$project$Main$GoToDoc = F2(
	function (a, b) {
		return {$: 'GoToDoc', a: a, b: b};
	});
var $author$project$Main$viewPackageVersions = function (model) {
	...
	if (_v0.$ === 'Just') {
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$Events$onClick(
										A2($author$project$Main$GoToDoc, n, v))
									]))
							]),
						_List_fromArray(
							[
		...
		var eHtml = A3(
			...
			A2(
				$elm$core$Maybe$withDefault,
				_List_Nil,
				A2($elm$core$Dict$get, n, model.entries)));
		return $rundis$elm_bootstrap$Bootstrap$ListGroup$ul(eHtml);
	} else {
		return $rundis$elm_bootstrap$Bootstrap$ListGroup$ul(_List_Nil);
	}
};
...
var $author$project$Main$viewReloadButton = function (model) {
	return A2(
...
var $author$project$Main$view = function (model) {
	var _v0 = model.is_loading;
	if (!_v0) {
		var versionHtml = $author$project$Main$viewPackageVersions(model);
		var packageHtml = $author$project$Main$viewPackageNames(model);
		var headerHtml = $author$project$Main$viewHeader(model);
		var buttonHtml = $author$project$Main$viewReloadButton(model);
		var contHtml = A2(
		...
		var progressHtml = $rundis$elm_bootstrap$Bootstrap$Progress$progress(
			_List_fromArray(
				[
					$rundis$elm_bootstrap$Bootstrap$Progress$value(model.n_ticks)
				]));
		var headerHtml = $author$project$Main$viewHeader(model);
		var contHtml = A2(
			$rundis$elm_bootstrap$Bootstrap$Grid$container,
